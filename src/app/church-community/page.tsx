...
function getVideoEmbedUrl(videoInfo: { platform: string; id: string }): string {
  if (videoInfo.platform === 'youtube') {
    return `https://www.youtube.com/embed/${videoInfo.id}?autoplay=1`;
  }
...
export default function ChurchCommunityPage() {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (pendingImages.length > 0) insertData.image_urls = pendingImages;
      if (videoUrlInput.trim() && parseVideoUrl(videoUrlInput.trim())) insertData.video_url = videoUrlInput.trim();
      let { error } = await supabase.from('church_community_posts').insert(insertData);
      // If insert fails due to missing media columns, retry without them
      if (error && (error.message?.includes('image_urls') || error.message?.includes('video_url'))) {
        const { image_urls, video_url, ...baseData } = insertData;
        const retryResult = await supabase.from('church_community_posts').insert(baseData);
        error = retryResult.error;
      }
      if (error) throw error;
      setNewForm({ title: '', body: '', category: 'announcement' }); setPendingImages([]); setVideoUrlInput(''); setShowNewPost(false); setPublishMsg('Published!');
      const { data: postsData } = await supabase.from('church_community_posts').select('*').eq('church_user_id', pastorId).order('created_at', { ascending: false });
      if (postsData) setPosts(postsData);
    ...
    try {
      ...
      for (const file of filesToProcess) {
        ...
        if (data.success && data.url) {
          setPendingImages(prev => [...prev, data.url].slice(0, 5));
        }
      }
    } catch (err) {
  ...
  // Render post media (images + video)
  const renderPostMedia = (post: Post, inModal: boolean = false) => {
      ...
      <>
        {imageUrls.length > 0 && (
          <div style={{
            ...
            maxHeight: inModal ? 'none' : '240px',
            overflow: 'hidden', borderRadius: '8px',
          }}>
            {imageUrls.map((url, idx) => (
              <div key={idx} style={{ position: 'relative', cursor: 'pointer', borderRadius: '6px', overflow: 'hidden' }}
        ...
        {videoInfo && (
          <div style={{ marginTop: '10px', position: 'relative', borderRadius: '8px', overflow: 'hidden', cursor: 'pointer' }}
            onClick={(e) => { e.stopPropagation(); setPlayingVideo(playingVideo === post.id ? null : post.id); }}>
  ...
  return (
    <div>
          ...
          {showNewPost ? (
            <div className="card" style={{ padding: '20px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                  <button
                    ...
                    style={{
                      ...
                      cursor: uploadingImage || pendingImages.length >= 5 ? 'not-allowed' : 'pointer',
                      fontSize: '13px', color: '#555',
                      opacity: uploadingImage || pendingImages.length >= 5 ? 0.5 : 1,
                    }}
                  >
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={() => { setShowNewPost(false); setPendingImages([]); setVideoUrlInput(''); }} className="btn-secondary">Cancel</button>
      ...
      {lightboxImage && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px', cursor: 'pointer' }} onClick={() => setLightboxImage(null)}>
          <img src={lightboxImage} alt="Full size" style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain', borderRadius: '8px' }} />
          <button onClick={() => setLightboxImage(null)} style={{ position: 'absolute', top: '20px', right: '20px', background: 'rgba(0,0,0,0.6)', color: 'white', border: 'none', borderRadius: '50%', width: '40px', height: '40px', fontSize: '20px', cursor: 'pointer' }}>✕</button>
        </div>
      )}
    </div>
  );
