...
const CATEGORIES = [
  { value: '', label: 'All / 全部' },
  { value: 'ministry-tips', label: '💡 Ministry Tips / 事工技巧' },
  { value: 'leadership', label: '👑 Leadership / 领导力' },
  { value: 'outreach', label: '🤝 Outreach / 外展' },
  { value: 'worship', label: '🎵 Worship / 敬拜' },
  { value: 'youth', label: '🧒 Youth Ministry / 青少年事工' },
  { value: 'tech', label: '💻 Church Tech / 教会科技' },
  { value: 'pastor-care', label: '❤️ Pastor Care / 牧者关怀' },
  { value: 'testimony', label: '🌟 Testimony / 见证' },
  { value: 'general', label: '📝 General / 综合' },
];

const SORT_OPTIONS = [
  { value: 'popular', label: 'Most Helpful / 最有帮助' },
...
export default function CommunityPage() {
  ...
  async function handlePublish() {
    if (!newForm.title.trim() || !newForm.body.trim()) {
      setPublishMsg('Title and content are required / 标题和内容必填');
      return;
    }
    setPublishing(true);
    setPublishMsg('');
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setPublishMsg('✅ Published! +50 points / 发布成功！+50积分');
        setNewForm({ title: '', body: '', category: 'general', tags: '' });
        loadPosts();
        setTimeout(() => { setShowNewPost(false); setPublishMsg(''); }, 2000);
      } else {
        setPublishMsg(data.error || 'Failed to publish / 发布失败');
      }
    } catch {
      setPublishMsg('Failed to publish / 发布失败');
    } finally {
      setPublishing(false);
    }
  }
  ...
  async function handleDelete(postId: string) {
    if (!confirm('Delete this post? / 确定删除？')) return;
    try {
    ...
    <div style={{ padding: mobile ? '16px' : '0' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: mobile ? 'flex-start' : 'center', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            <h1 style={{ fontSize: mobile ? '22px' : '28px', fontWeight: 'bold', color: 'var(--primary)', marginBottom: '8px' }}>
              🌍 Community Knowledge Base 社区知识库
            </h1>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', flexWrap: 'wrap' }}>
        <input
          ...
          placeholder="Search tips, ideas, experiences... / 搜索技巧、想法、经验..."
      ...
      <div style={{ display: 'flex', gap: '16px', marginBottom: '24px', flexWrap: 'wrap' }}>
        <div style={{ background: 'linear-gradient(135deg, #eff6ff, #dbeafe)', borderRadius: '12px', padding: '16px 20px', flex: 1, minWidth: '140px' }}>
          ...
          <div style={{ fontSize: '13px', color: '#666' }}>Posts / 帖子</div>
        </div>
        <div style={{ background: 'linear-gradient(135deg, #fef3c7, #fde68a)', borderRadius: '12px', padding: '16px 20px', flex: 1, minWidth: '140px' }}>
          ...
          <div style={{ fontSize: '13px', color: '#666' }}>Helpful / 有帮助</div>
        </div>
        <div style={{ background: 'linear-gradient(135deg, #f0fdf4, #bbf7d0)', borderRadius: '12px', padding: '16px 20px', flex: 1, minWidth: '140px' }}>
          ...
          <div style={{ fontSize: '13px', color: '#666' }}>Points per post / 每帖积分</div>
        </div>
      </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '16px' }}>
          <div style={{ background: 'white', borderRadius: '16px', padding: mobile ? '20px' : '32px', maxWidth: '640px', width: '100%', maxHeight: '90vh', overflow: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#1e3a5f' }}>✍️ Share Your Wisdom 分享你的智慧</h2>
            ...
            <p style={{ color: '#999', fontSize: '13px', marginBottom: '20px' }}>
              分享事工技巧、领导力心得、外展想法，或任何能祝福其他牧师的内容。分享可获得<strong>50积分</strong>！
            </p>
            ...
            >
              {publishing ? 'Publishing... 发布中...' : '✨ Publish & Earn 50 Points 发布并获50积分'}
            </button>
          </div>
        </div>
      )}
