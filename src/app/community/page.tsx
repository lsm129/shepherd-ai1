...
export default function CommunityPage() {
 ...
 function getVideoThumbnail(vi: { platform: string; id: string }): string {
   return vi.platform === 'youtube'
     ? `https://img.youtube.com/vi/${vi.id}/hqdefault.jpg`
     : `https://vumbnail.com/${vi.id}.jpg`;
 }

 function getVideoEmbedUrl(vi: { platform: string; id: string }): string {
   return vi.platform === 'youtube'
     ? `https://www.youtube.com/embed/${vi.id}?autoplay=1`
     : `https://player.vimeo.com/video/${vi.id}?autoplay=1`;
 }
 ...
 async function compressImage(file: File, maxWidth: number = 1200, quality: number = 0.8): Promise<Blob> {
   return new Promise((resolve, reject) => {
     ...
     img.onload = () => {
       ...
       canvas.toBlob((blob) => { if (blob) resolve(blob); else reject(new Error('Compression failed')); },
 ...
 async function handleImageSelect(e: React.ChangeEvent<HTMLInputElement>) {
   ...
   const filesToProcess = Array.from(files).slice(0, 5 - pendingImages.length);
   ...
   try {
     ...
     for (const file of filesToProcess) {
       ...
       let blobToUpload: Blob = file.type === 'image/gif' ? file : await compressImage(file);
       const formData = new FormData();
       formData.append('file', new File([blobToUpload], file.name, { type: blobToUpload.type || file.type }));
       formData.append('userId', session.user.id);
       const res = await fetch('/api/community/upload-image', { method: 'POST', body: formData });
       const data = await res.json();
       if (data.success && data.url) setPendingImages(prev => [...prev, data.url].slice(0, 5));
     }
 ...
 function renderPostMedia(post: CommunityPost, inModal: boolean = false) {
   const imageUrls = post.image_urls || [];
   const videoUrl = post.video_url;
   const videoInfo = videoUrl ? parseVideoUrl(videoUrl) : null;
   return (<>
     {imageUrls.length > 0 && (
       <div style={{ display: 'grid', gridTemplateColumns: imageUrls.length === 1 ? '1fr' : 'repeat(2, 1fr)', gap: '6px', marginTop: '10px', maxHeight: inModal ? 'none' : '240px', overflow: 'hidden', borderRadius: '8px' }}>
         {imageUrls.map((url: string, idx: number) => (
           <div key={idx} style={{ position: 'relative', cursor: 'pointer', borderRadius: '6px', overflow: 'hidden' }} onClick={(e) => { e.stopPropagation(); setLightboxImage(url); }}>
             <img src={url} alt={`Image ${idx + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', borderRadius: '6px' }} loading="lazy" />
           </div>
         ))}
       </div>
     )}
     {videoInfo && (
       <div style={{ marginTop: '10px', position: 'relative', borderRadius: '8px', overflow: 'hidden', cursor: 'pointer' }} onClick={(e) => { e.stopPropagation(); setPlayingVideo(playingVideo === post.id ? null : post.id); }}>
         {playingVideo === post.id ? (
           <iframe src={getVideoEmbedUrl(videoInfo)} style={{ width: '100%', aspectRatio: '16/9', border: 'none', borderRadius: '8px' }} allow="autoplay; encrypted-media" allowFullScreen />
         ) : (
           <div style={{ position: 'relative', width: '100%', aspectRatio: '16/9', background: '#000', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
             <img src={getVideoThumbnail(videoInfo)} alt="Video thumbnail" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'cover', borderRadius: '8px' }} loading="lazy" />
             <div style={{ position: 'relative', zIndex: 1, width: '56px', height: '56px', borderRadius: '50%', background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '3px solid white' }}>
 ...
 setNewForm({ title: '', body: '', category: 'general', tags: '' }); setPendingImages([]); setVideoUrlInput('');
 loadPosts();
 ...
 useEffect(() => {
   const handler = (event: ErrorEvent) => {
     console.error('Community page error:', event.error);
     setPageError(event.error?.message || String(event.error));
   };
   ...
   return (
     <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8fafc', padding: '24px' }}>
       <div style={{ background: 'white', borderRadius: '16px', padding: '32px', maxWidth: '500px', width: '100%', textAlign: 'center' }}>
         ...
         <h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#1e3a5f', marginBottom: '8px' }}>Community Page Error</h2>
         ...
         <button onClick={() => { setPageError(''); window.location.reload(); }} style={{ background: '#1e3a5f', color: 'white', border: 'none', borderRadius: '8px', padding: '10px 24px', fontWeight: '600', cursor: 'pointer', fontSize: '14px' }}>
           Reload Page
         </button>
       </div>
     </div>
 ...
 const getCategoryEmoji = (cat?: string) => {
    switch (cat) {
      ...
      case 'praise': return '🙌';
      case 'ministry-tips': return '💡';
      case 'leadership': return '👑';
      case 'outreach': return '🤝';
      case 'worship': return '🎵';
      case 'youth': return '🧒';
      case 'tech': return '💻';
      case 'pastor-care': return '❤️';
 ...
 const getCategoryLabel = (cat?: string) => {
    switch (cat) {
      case 'prayer-request': return 'Prayer Request';
      case 'praise': return 'Praise & Thanks';
      case 'ministry-tips': return 'Ministry Tips';
      case 'leadership': return 'Leadership';
      case 'outreach': return 'Outreach';
      case 'worship': return 'Worship';
      case 'youth': return 'Youth Ministry';
      case 'tech': return 'Church Tech';
      case 'pastor-care': return 'Pastor Care';
 ...
 return (
 <div className="notranslate" style={{ padding: mobile ? '16px' : '0' }}>
 {/* Header */}
 ...
 <button onClick={() => { setShowNewPost(false); setPublishMsg(''); setPendingImages([]); setVideoUrlInput(''); }} style={{ background: 'none', border: 'none', fontSize: '24px', cursor: 'pointer', color: '#999' }}>×</button>
 </div>
 ...
 <div style={{ marginBottom: '16px' }}>
   ...
   <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
     <button
       ...
       disabled={uploadingImage || pendingImages.length >= 5}
       style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 14px', borderRadius: '8px', border: '1px solid #e5e7eb', background: 'white', cursor: uploadingImage || pendingImages.length >= 5 ? 'not-allowed' : 'pointer', fontSize: '13px', color: '#555', opacity: uploadingImage || pendingImages.length >= 5 ? 0.5 : 1 }}
     >
       📷 {uploadingImage ? 'Uploading...' : 'Add Image'} {pendingImages.length > 0 ? `(${pendingImages.length}/5)` : ''}
     </button>
   </div>
     ...
     <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '6px' }}>
       {pendingImages.map((url, idx) => (
         <div key={idx} style={{ position: 'relative', borderRadius: '8px', overflow: 'hidden', aspectRatio: '1' }}>
           ...
           <button onClick={() => removePendingImage(idx)} style={{ position: 'absolute', top: '2px', right: '2px', width: '20px', height: '20px', borderRadius: '50%', background: 'rgba(0,0,0,0.6)', color: 'white', border: 'none', cursor: 'pointer', fontSize: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
         </div>
       ))}
     </div>
   )}
 ...
 <div style={{ marginBottom: '16px' }}>
   ...
   <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
     ...
     />
     {videoUrlInput && !parseVideoUrl(videoUrlInput) && (
       <span style={{ fontSize: '11px', color: '#ef4444' }}>Invalid URL</span>
     )}
   </div>
   {videoUrlInput && parseVideoUrl(videoUrlInput) && (
     <div style={{ marginTop: '8px', position: 'relative', borderRadius: '8px', overflow: 'hidden', maxWidth: '300px' }}>
       <img src={getVideoThumbnail(parseVideoUrl(videoUrlInput)!)} alt="Video preview" style={{ width: '100%', aspectRatio: '16/9', objectFit: 'cover', borderRadius: '8px' }} />
       <button onClick={() => setVideoUrlInput('')} style={{ position: 'absolute', top: '4px', right: '4px', width: '24px', height: '24px', borderRadius: '50%', background: 'rgba(0,0,0,0.6)', color: 'white', border: 'none', cursor: 'pointer', fontSize: '12px' }}>✕</button>
     </div>
   )}
 </div>
 {publishMsg && (
 ...
 {renderPostMedia(selectedPost, true)}
 ...
 {/* Lightbox */}
 {lightboxImage && (
   <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, cursor: 'pointer' }} onClick={() => setLightboxImage(null)}>
     <img src={lightboxImage} alt="Full size" style={{ maxWidth: '90vw', maxHeight: '90vh', objectFit: 'contain', borderRadius: '8px' }} />
     <button style={{ position: 'absolute', top: '16px', right: '16px', width: '40px', height: '40px', borderRadius: '50%', background: 'rgba(255,255,255,0.2)', color: 'white', border: 'none', fontSize: '20px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
   </div>
 )}
 </div>
 );
