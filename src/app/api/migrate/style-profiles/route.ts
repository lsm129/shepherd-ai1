...
export async function POST(request: NextRequest) {
  try {
    ...
    // Try to create via SQL through the database URL
    ...
    if (databaseUrl) {
      try {
        ...
        await client.end();
        return NextResponse.json({ status: 'created', message: 'ai_style_profiles table created successfully' });
      } catch (pgErr: any) {
        console.error('Direct SQL creation failed:', pgErr.message);
      }
    }

    // Fallback: return SQL for manual creation
    return NextResponse.json({
      status: 'needs_creation',
      message: 'Please run this SQL in Supabase SQL Editor to create the ai_style_profiles table:',
