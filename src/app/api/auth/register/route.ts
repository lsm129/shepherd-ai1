...
export async function POST(req: NextRequest) {
  try {
      ...
      try {
        ...
        await fetch(`${SUPABASE_URL}/rest/v1/church_settings`, {
          ...
          body: JSON.stringify({
            ...
            worship_style: metadata.worship_style, address: fullAddress, reply_email: email,
          }),
        });
    ...
    // Track partner referral if ref parameter matches a partner code
    if (metadata?.referred_by && userId) {
      try {
        // Check if the referral code matches a partner (not a church code)
        const partnerCheckRes = await fetch(
          `${SUPABASE_URL}/rest/v1/partners?referral_code=eq.${encodeURIComponent(metadata.referred_by)}&select=id,status`,
          { headers: { "apikey": SUPABASE_SERVICE_KEY, "Authorization": `Bearer ${SUPABASE_SERVICE_KEY}` } }
        );
        const partnerData = await partnerCheckRes.json();
        if (partnerData?.length > 0 && partnerData[0].status === 'active') {
          // This is a partner referral - track it
          const churchName = metadata?.church_name || null;
          await fetch(`${SUPABASE_URL}/rest/v1/partner_referrals`, {
            ...
            body: JSON.stringify({
              ...
              referred_church: churchName,
              status: 'trial',
            }),
          });
          console.log('Partner referral tracked for code:', metadata.referred_by);
        }
      } catch (e) { console.error('Partner referral tracking error:', e); }
    }
