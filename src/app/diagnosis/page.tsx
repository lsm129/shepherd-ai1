    ...
    (async () => {
      try {
        ...
        const [settingsRes, subRes] = await Promise.allSettled([
          supabase.from('church_settings').select('*').eq('user_id', session.user.id).single(),
          fetch('/api/subscription?userId=' + session.user.id).then(r => r.ok ? r.json() : null).catch(() => null),
        ]);

        // Mark diagnosis as viewed (don't await - non-critical)
        supabase.auth.updateUser({ data: { diagnosis_pending: false } }).catch(() => {});

        // Get plan from subscription API
        if (subRes.status === 'fulfilled' && subRes.value?.plan) {
          setPlan(subRes.value.plan);
        }

        // Get church settings
        const cs = (settingsRes.status === 'fulfilled' && settingsRes.value?.data) || {};
        ...
        const dims: Dimension[] = [
          ...
          { key: 'engagement', label: 'Congregation Engagement', icon: '🙏', score: engagement,
            ...
            feature: 'Prayer Requests', featureHref: '/prayer-requests', featurePlan: 'free' },
          { key: 'digital', label: 'Digital Presence', icon: '🌐', score: digital,
            suggestion: 'Weekly Newsletter builds your church\'s online presence automatically.',
            feature: 'Weekly Newsletter', featureHref: '/weekly-newsletter', featurePlan: 'starter' },
  ...
  return (
    <div>
      <div style={{ marginBottom: '32px' }}>
        <h1 style={{ fontSize: '28px', fontWeight: 'bold', color: '#1a202c', marginBottom: '8px' }}>🏥 Ministry Health Report</h1>
        <p style={{ color: '#64748b' }}>{churchName ? `${churchName} — ` : ''}Personalized recommendations to strengthen your ministry</p>
      </div>
      ...
      <div className="card" style={{ textAlign: 'center', marginBottom: '24px', padding: '32px' }}>
        ...
        <div style={{ fontSize: '18px', color: '#64748b', marginBottom: '16px' }}>Overall Ministry Health Score</div>
          ...
          <div key={dim.key} className="card" style={{ padding: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div>
                  <div style={{ fontWeight: 'bold', fontSize: '16px', color: '#1a202c' }}>{dim.label}</div>
