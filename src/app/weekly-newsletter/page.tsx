...
export default function WeeklyNewsletterPage() {
   ...
   if (userId) {
     try {
       await fetch('/api/ai-habits', {
         ...
         body: JSON.stringify({
           userId,
           action: isEditing ? 'edit_approve' : 'approve',
           platform: 'newsletter',
           originalText: newsletter?.content || '',
 ...
 try {
   ...
   if (!session?.user) {
     setError('Please log in to send newsletters');
     setSending(false);
     return;
   }

   // Actually send emails to all church members
   const response = await fetch('/api/email/newsletter-send', {
     method: 'POST',
     headers: { 'Content-Type': 'application/json' },
     body: JSON.stringify({
       pastorUserId: session.user.id,
       title: editedTitle || newsletterTitle,
 ...
 {sendResults ? (
   <>
     <p style={{ color: 'var(--text-secondary)', marginBottom: '8px' }}>
       Successfully sent to <strong>{sendResults.sent}</strong> of <strong>{sendResults.totalMembers}</strong> church members
     </p>
     {sendResults.failed > 0 && (
       <p style={{ color: '#ef4444', marginBottom: '16px' }}>{sendResults.failed} emails failed to send</p>
     )}
     {sendResults.sent === 0 && sendResults.message && (
       <p style={{ color: '#f59e0b', marginBottom: '16px' }}>{sendResults.message}</p>
     )}
   </>
 ) : (
   <p style={{ color: 'var(--text-secondary)', marginBottom: '32px' }}>Your newsletter has been saved.</p>
 )}
