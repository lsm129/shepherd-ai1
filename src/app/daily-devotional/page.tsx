...
export default function DailyDevotionalPage() {
  ...
  // Congregants can access devotional free; pastors need Starter+
 ...
 if (session?.user) {
          try {
            ...
            if (d.role === 'congregant') setUserRole('congregant');
          } catch(e) {}
 ...
 setEmailVerified(!!session.user.email_confirmed_at);
  ...
  try {
    const subRes = await fetch('/api/email/subscribe-devotional?userId=' + session.user.id);
    const subData = await subRes.json();
    if (subData.subscribed) setSubscribed(true);
  } catch(e) {}
 }
 } catch {}
 })();
 ...
 if (params.get('unsubscribe') === 'true') {
   // Will handle after userId is set via a separate effect
 }
 ...
 useEffect(() => {
   if (!userId) return;
   const params = new URLSearchParams(window.location.search);
   if (params.get('unsubscribe') === 'true' && subscribed) {
     handleToggleSubscription(false);
   }
 }, [userId]);

 // Devotional is free for all users - no plan gate
  if (!mounted) return null;

 async function handleToggleSubscription(subscribe: boolean) {
   if (!userId) return;
   setSubLoading(true);
   try {
     const res = await fetch('/api/email/subscribe-devotional', {
       method: 'POST',
       headers: { 'Content-Type': 'application/json' },
       body: JSON.stringify({ userId, subscribe }),
     });
     ...
     console.error('Subscription toggle error:', e);
   } finally {
     setSubLoading(false);
   }
 }
   ...
   if (userId) {
     try {
       await fetch('/api/ai-habits', {
         ...
         body: JSON.stringify({
           userId,
           action: editingField ? 'edit_approve' : 'approve',
           platform: 'devotional',
           originalText: devotional?.content || '',
           editedText: editingField ? text : undefined,
           toolType: 'devotional',
         }),
       });
 ...
 {userId && (
   <div style={{
     ...
     border: subscribed ? '1px solid #86efac' : '1px solid #93c5fd',
     borderRadius: '12px',
     ...
     <div>
       <h3 style={{ fontSize: '16px', fontWeight: '600', margin: '0 0 4px 0', color: subscribed ? '#166534' : '#1e3a5f' }}>
         📬 Daily Devotional Email
       </h3>
       <p style={{ fontSize: '14px', color: 'var(--text-secondary)', margin: 0 }}>
         {subscribed
           ? 'You\'ll receive a personalized devotional every morning.'
           : 'Get a daily devotional delivered to your inbox every morning.'}
       </p>
     </div>
     <button
       onClick={() => handleToggleSubscription(!subscribed)}
       disabled={subLoading}
       style={{
         padding: '10px 20px',
         borderRadius: '8px',
         ...
         fontWeight: '600',
         fontSize: '14px',
         background: subscribed ? '#dcfce7' : 'linear-gradient(135deg, #1e3a5f 0%, #4a90a4 100%)',
         color: subscribed ? '#166534' : 'white',
         transition: 'all 0.2s',
         whiteSpace: 'nowrap',
       }}
     >
       {subLoading ? '...' : subscribed ? '✅ Subscribed' : 'Subscribe to Daily Email'}
     </button>
   </div>
 )}
 ...
 <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
       <button onClick={handleApproveAndCopy} className="btn-primary" style={{ flex: 1 }}>
         {approved ? '✓ Copied!' : copied ? '✓ Copied!' : '👍 Approve & Copy'}
       </button>
       <button onClick={handleReject} style={{ flex: 1, background: '#fee2e2', color: 'var(--error)', border: '1px solid #fca5a5', borderRadius: '8px', padding: '10px', cursor: 'pointer', fontWeight: '600', fontSize: '14px' }}>
         🔄 Try Again
       </button>
