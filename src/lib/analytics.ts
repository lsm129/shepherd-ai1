...
/** Track a page view — writes to both page_views and analytics_events tables */
export function trackPageView(pageName?: string) {
  const name = pageName || (typeof document !== 'undefined' ? document.title : '');
  queueEvent('pageview', { pageName: name });
  queueEvent('event', { eventName: 'page_view', eventData: { pageName: name } });
}
