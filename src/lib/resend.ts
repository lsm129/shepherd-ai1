...
// 发送邮件序列（用于访客跟进等场景）
// Week 1 sent immediately, weeks 2-6 saved to scheduled_emails for cron to send later
export async function sendEmailSequence({
  ...
  for (const email of emails) {
    ...
    const senderFrom = fromEmail ? `${fromName || 'Church'} <${fromEmail}>` : `${fromName || 'ShepherdAI'} <hello@shepherdaitech.com>`;
    ...
    if (isFirstEmail) {
      ...
      // Save weeks 2-6 as scheduled emails (will be sent by cron job)
      results.push({
        week: email.week,
        messageId: null,
        scheduledFor: scheduledDate.toISOString(),
